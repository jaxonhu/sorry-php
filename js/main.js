...
function submit(template) {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4) {
            if (this.status == 521 || this.status == 404) {
                ...
                var inner = "<a target='_blank' href='http://localhost/sorry/"+ this.responseText+"'>点击下载</a>";
    ...
    if (wechat_checkbox && wechat_checkbox.checked) {
        ...
        xhttp.open("POST", "../make.php", true);        
    }
    xhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    xhttp.send(make_body(template));
}
